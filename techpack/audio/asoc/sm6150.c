...
struct msm_asoc_mach_data {
	...
	/* Huaqin add for smartPA config mi2s by xuyuqing at 2019/01/28 start*/
	struct device_node *sec_mi2s_gpio_p;
	/* Huaqin add for smartPA config mi2s by xuyuqing at 2019/01/28 end*/
...
static struct dev_config mi2s_tx_cfg[] = {
	...
	/* Huaqin add for integrate smartPA cal function by xuyuqing at 2019/01/29 start */
	[SEC_MI2S]  = {SAMPLING_RATE_48KHZ, SNDRV_PCM_FORMAT_S16_LE, 2},
	/* Huaqin add for integrate smartPA cal function by xuyuqing at 2019/01/29 end */
...
static struct wcd_mbhc_config wcd_mbhc_cfg = {
	...
	/* Huaqin modify for JD2020-217 config button by xuyuqing at 2019/01/26 start*/
	.key_code[1] = KEY_VOLUMEUP,
	.key_code[2] = KEY_VOLUMEDOWN,
	.key_code[3] = 0,
	/* Huaqin modify for JD2020-217 config button by xuyuqing at 2019/01/26 end*/
	.key_code[4] = 0,
	.key_code[5] = 0,
	.key_code[6] = 0,
	.key_code[7] = 0,
...
static int msm_int_audrx_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	if (rtd->card->num_aux_devs &&
		...
		list_for_each_entry(aux_comp, &rtd->card->aux_comp_list,
				card_aux_list) {
		/* Huaqin modify for JD2020-214 disable wsa to register sound card by xuyuqing at 2019/01/26 start*/
     	if (0){
			if (!strcmp(aux_comp->name, WSA8810_NAME_1) ||
			    !strcmp(aux_comp->name, WSA8810_NAME_2)) {
		...
		/* Huaqin modify for JD2020-214 disable wsa to register sound card by xuyuqing at 2019/01/26 end*/
	}
	card = rtd->card->snd_card;
	if (!pdata->codec_root) {
		entry = snd_info_create_subdir(card->module, "codecs",
...
#define S(X, Y) ((WCD_MBHC_CAL_PLUG_TYPE_PTR(wcd_mbhc_cal)->X) = (Y))
	/* Huaqin modify for JD2020-217 config headset by xuyuqing at 2019/01/26 start*/
	S(v_hs_max, 1700);
	/* Huaqin modify for JD2020-217 config headset by xuyuqing at 2019/01/26 end*/
#undef S
...
static int msm_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
	...
	if (++mi2s_intf_conf[index].ref_cnt == 1) {
		...
		/* Huaqin add for smartPA config mi2s by xuyuqing at 2019/01/28 start*/
		if (index == SEC_MI2S) {
			ret_pinctrl = msm_cdc_pinctrl_select_active_state(pdata->sec_mi2s_gpio_p);
			if (ret_pinctrl)
...
static void msm_mi2s_snd_shutdown(struct snd_pcm_substream *substream)
{
	...
	if (--mi2s_intf_conf[index].ref_cnt == 0) {
		...
		/* Huaqin add for smartPA config mi2s by xuyuqing at 2019/01/28 start*/
		if (index == SEC_MI2S) {
			ret_pinctrl = msm_cdc_pinctrl_select_sleep_state(pdata->sec_mi2s_gpio_p);
			if (ret_pinctrl)
				pr_err("%s: MI2S TLMM pinctrl set failed with %d\n", __func__, ret_pinctrl);
		}
		/* Huaqin add for smartPA config mi2s by xuyuqing at 2019/01/28 end*/
...
/* Huaqin add for smartPA config dai_link by xuyuqing at 2019/01/28 start*/
...
/* Huaqin add for smartPA config dai_link by xuyuqing at 2019/01/28 end*/
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	if (!pdata->sec_mi2s_gpio_p) {
		dev_err(&pdev->dev, "property %s not detected in node %s\n",
			"qcom,sec-mi2s-gpios", pdev->dev.of_node->full_name);
	}
	/* Huaqin add for smartPA config mi2s by xuyuqing at 2019/01/28 end*/
